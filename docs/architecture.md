...
**3D/motion upgrade (added 2026-08-18)**: driven by a developer-supplied instruction (`LORDGEN_3D_WEBSITE_UPGRADE_INSTRUCTION.md`) and a companion project Skill (`.claude/skills/lordgen-3d-web-experience/SKILL.md`). Added a bounded, hero-only WebGL scene (`website/hero3d.js`) rendering the logo mark's own 5-bar rhythm as a slowly rotating, pointer-reactive cluster, plus event-driven interaction polish (`website/motion.js`: panel tilt, magnetic hero CTAs, scroll-reveal) and a page-wide (not just hero) low-opacity depth treatment in `styles.css`. One new vendored dependency: `website/vendor/three.min.js`, pinned at **r160 specifically** (not latest) because three.js dropped its global/UMD build at r161, and `<script type="module">` is blocked by CORS when `index.html` is opened via `file://` — this site's primary, documented run mode — confirmed live during implementation. Full rationale in `website/vendor/THREE_LICENSE.txt` and `website/README.md`. Both new scripts bail cleanly (no console errors) under `prefers-reduced-motion: reduce`, mobile widths (≤860px), missing WebGL, or coarse pointer input — the existing CSS-only hero background is the entire fallback in every case, nothing separate was built for it. Zero-radius/flush-left brand discipline preserved throughout; no framework or build step introduced.

Verification status: file:// mode confirmed clean (console, full intake→approval→send interaction flow, mobile breakpoint hiding the 3D layer, workflow-viz active/done arrow states via computed styles) via chrome-devtools MCP. Reduced-motion JS-gate confirmation, served (`http://`) mode parity, panel hover/focus screenshots, and a Lighthouse pass were interrupted mid-session by an MCP reconnect and are still open — see "Known open items."

## Known open items

- 3D/motion upgrade verification incomplete: reduced-motion JS-gate confirmation (`hero3d.js`/`motion.js` never initializing under a forced `prefers-reduced-motion: reduce`), served (`http://`) mode parity with the already-confirmed `file://` mode, `.panel` hover/focus-within screenshots, and a Lighthouse accessibility/contrast pass are still pending — interrupted by a chrome-devtools MCP disconnect mid-session, not by a defect found. Resume via the verification checklist in `.claude/plans/continue-clever-lightning.md` once that MCP server reconnects, before treating this upgrade as done.
