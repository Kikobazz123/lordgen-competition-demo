...
**To resume**: read the plan file above (or this summary if it's gone), then start with the two n8n workflows (I was mid-way through checking `list_credentials`/header-auth setup when the session was checkpointed), then the serverless function, then the website changes, then hand off Vercel env-var configuration to the developer, then run the full verification pass listed in the plan (n8n MCP checks, chrome-devtools MCP checks, a negative/reliability check, secret hygiene, and the addendum §25 acceptance script end to end).
