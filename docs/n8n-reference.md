...
Connection status: **connected** (confirmed 2026-08-15 via `search_workflows`; re-confirmed live 2026-08-21). The instance now holds 6 workflows: the current live system (5 workflows -- Diagnostic, Issue Proposal, Approval Decision, Approve, Handover -- all `active: true`, exported to `workflows/live-*.json`, see `workflows/README.md` for the full chain and IDs) plus the original, now-superseded `055TNXGtfItIgqf1` (`workflows/competition-demo.json`, unpublished). See root `README.md` / `.env.example` for setup steps if reconnecting in a new environment.
